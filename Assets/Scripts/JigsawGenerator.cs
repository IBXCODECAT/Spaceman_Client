...
namespace BlueScreenStudios.JigsawSystem
{
    public class JigsawGenerator : MonoBehaviour
    {
        ...
        [Tooltip("Assign a starting jigsaw piece here. This is the piece that will be used to start the generation.")]
        [SerializeField] JigsawPiece startPiece;

        [Header("Piece Pools")]
        [Tooltip("Assign all jigsaw pieces here that belong in the cooridors pool.")]
        [SerializeField] JigsawPiece[] cooridorPieces;
        [Tooltip("Assign all jigsaw pieces here that belong in the branches pool.")]
        [SerializeField] JigsawPiece[] branchPieces;
        [Tooltip("Assign all jigsaw pieces here that belong in the decorations pool.")]
        ...
        /// <summary>
        /// The JigsawGenerator uses pools to distinguish piece types.
        /// </summary>
        public enum GeneratorPool { Cooridors, Branches, Decorations, Terminators };
            ...
            for(int i = 0; i < generationSteps; i++)
            {
                ...
                foreach(JigsawHook outboundHook in unconnectedHooks)
                {
                    ...
                    if(!outboundHook.connected)
                    {
                        ...
                        JigsawPiece piecePrefab = SelectNextPiece(outboundHook.primaryConnectionPool);
        ...
        {
            Vector3 inboundHookRotation = inboundHook.transform.eulerAngles;
            Vector3 outboundHookRotation = outboundHook.transform.eulerAngles;

            float hookRotationOffset = outboundHookRotation.y - inboundHookRotation.y;

            placedPiece.transform.eulerAngles = previousPiece.transform.eulerAngles;
            ...
            {
                if (hookRotationOffset == 0 || hookRotationOffset == 360)
                {
